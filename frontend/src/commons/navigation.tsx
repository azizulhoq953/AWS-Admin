...
const defaultOnFollowHandler: SideNavigationProps['onFollow'] = (event) => {
  // Prevent default navigation for demo pages
  event.preventDefault();
};
...
interface NavigationProps {
  ...
  header?: SideNavigationProps['header'];
  items?: SideNavigationProps['items'];
  onFollow?: SideNavigationProps['onFollow'];  // Use onFollow here
}
...
export function Navigation({
  ...
  header = navHeader,
  items = navItems,
  onFollow = defaultOnFollowHandler,  // Use onFollow prop here
}: NavigationProps) {
  return (
    <SideNavigation items={items} header={header} activeHref={activeHref} onFollow={onFollow} />
  );
}
